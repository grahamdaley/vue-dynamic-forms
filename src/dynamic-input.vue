<template>
	<div class="dynamic-input-wrapper">
		<label v-if="input.type !== 'checkbox'" v-show="input.label" :for="input.name" >{{input.label}}</label>
		...
		<!-- Checkbox -->
		<div v-else-if="input.type == 'checkbox'" style="display:inline-flex">
			<input :class="input.class" type="checkbox" style="display:inline;width:10%;float:left;margin-top:3px;" :name="input.name" v-model="input.value">
			<label :for="input.name" style="display:inline">{{input.label}}</label>
		</div>
